...
<h4><b><center>Los Eventos de Todos:</h4></b></center>
...
<th>Puntos_claves</th>-->
<th>Quien Creo el Evento</th>
<th>Visualizar Evento</th>
<th>Correo</th>
<th>Visualizar Temática</th>
<th>Visualizar Participantes</th>
<th>Visualizar Tiquetes</th>
  ...
  @foreach($evento as $row)
    <tr>
         ...
         <td><?php $comment = App\User::find($row->id_user); echo $comment->name; ?></td>
          <!--<td>{{$row->Lugar}}</td>
...
-->
 ...
 <!--<td><center><a href="{{ $url = route('eventos_general.edit', $row->id) }}" class="glyphicon glyphicon-envelope" aria-hidden="true"> </a></center></td>-->
  ...
  <td><center><a href="{{$url = route('eventos_general.show', $row->id) }}" class="glyphicon glyphicon-eye-open" aria-hidden="true"> </a></center></td>
